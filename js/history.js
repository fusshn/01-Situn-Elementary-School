            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                newsItems.forEach(item => {
                    item.addEventListener('click', function () {
                        // 移除所有 .news-item 的 active 類
                        newsItems.forEach(i => i.classList.remove('active'));
                        // 添加 active 類到被點擊的 .news-item
                        this.classList.add('active');

                        // 隱藏所有 .news-card
                        newsCards.forEach(card => card.classList.remove('active'));
                        // 隱藏所有 .history
                        historyDivs.forEach(div => div.classList.remove('active'));

                        // 顯示對應的 .news-card 和 .history
                        ...
                        historyDivs.forEach(div => {
                            if (div.getAttribute('data-target') === target) {
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                for (var i = 0; i < accButtons.length; i++) {
                    accButtons[i].addEventListener('click', function () {
                        // 切換當前按鈕的狀態
                        var activeButtons = document.getElementsByClassName('accordion-button');
